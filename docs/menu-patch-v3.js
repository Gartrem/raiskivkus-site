(() => {
  ...
  const cacheVersion = "20260917-4";
  window.__RAISKI_MENU_PATCH_VERSION__ = cacheVersion;
  ...
  const applyMenuChanges = (source) => {
    const products = source
      ...
      .map((product) => {
        ...
        // These are explicit final prices and must override the generic +20 rule above.
        if (next.slug === "roll-ikura") {
          next.price = 360;
        }

        if (next.slug === "roll-drakon") {
          next.price = 340;
        }
  ...
  window.fetch = async (...args) => {
    ...
    if (typeof input === "string") {
      requestArgs[0] = `${input}${input.includes("?") ? "&" : "?"}v=${cacheVersion}`;
    } else if (input instanceof Request) {
      const requestUrl = `${input.url}${input.url.includes("?") ? "&" : "?"}v=${cacheVersion}`;
      requestArgs[0] = new Request(requestUrl, input);
    }
    requestArgs[1] = { ...(args[1] || {}), cache: "no-store" };
